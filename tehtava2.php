...
<?php
	...
	$teht2 = pg_fetch_row($tehtavat, $_SESSION['monesko']);
    $sessio_id = $_SESSION['sessioid'];

	// Haetaan tehtävä
	$teht2 = $teht2[1];
    $tehtava = pg_query("SELECT kuvaus FROM oppimisymparisto.tehtavat WHERE Id='$teht2'");
	if (!$tehtava) {
	...
	$vastaus = pg_query("SELECT Esim_vastaus FROM oppimisymparisto.tehtavat WHERE Id = '$teht2'");
	if (!$vastaus) {
		echo "Virhe kyselyssä.\n";
		exit;
	}
	...
	if(isset($_POST['syote1'])){
	    ...
	    if (!pg_query($syote_muuttunut)) {
			...
			$_SESSION['yrityksia'] = $_SESSION['yrityksia'] + 1;
	     	$yrityksia = $_SESSION['yrityksia'];
	     	pg_query("INSERT INTO oppimisymparisto.yritys VALUES ('$sessio_id', '$teht2', '$yrityksia')");
	     	$riveja = pg_query("SELECT * FROM oppimisymparisto.yritys WHERE Sessio_id = $sessio_id AND Tehtavan_id = $teht2");
		 	if(pg_num_rows($riveja) > 2){
		 		$_SESSION['yrityksia'] = 0;
		 		$_SESSION['monesko']=$_SESSION['monesko']+1;
		 		header('Location: tehtava3.php');
		 	}
	    }
	    else{
...
<html>
    ...
    <body>
		...
		<?php
			...
			echo "<br />\n";
		?>
        <form method="post" action="tehtava2.php">
